...
function formatValue(v, key) {
  ...
  if (typeof v === "boolean") {
    return v
      ? '<span class="font-mono text-[var(--ink-added)] font-bold">true</span>'
      : '<span class="font-mono text-[var(--ink-removed)] font-bold">false</span>';
  }
  if (typeof v === "number") {
    ...
    if (key === "http.status_code" || key === "status_code") {
      if (v === 200) return '<span class="font-mono font-bold text-[var(--ink-added)] bg-[var(--ink-added-bg)] px-1.5 py-0.5 rounded">200 OK</span>';
      if (v === 500) return '<span class="font-mono font-bold text-[var(--ink-semantic)] bg-[var(--ink-semantic-bg)] px-1.5 py-0.5 rounded">500 Error</span>';
      if (v >= 400 && v < 500) return '<span class="font-mono font-bold text-[var(--ink-noise)] bg-[var(--ink-noise-bg)] px-1.5 py-0.5 rounded">' + v + '</span>';
...
function formatDelta(vA, vB, key) {
  if (vA === undefined || vB === undefined) return "";

  // 1. Check HTTP status code FIRST
  if (key === "http.status_code" || key === "status_code") {
    if (vA !== vB) {
      return '<span class="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-[var(--ink-semantic-bg)] text-[var(--ink-semantic)] border border-[var(--ink-semantic)]">STATUS CHANGE</span>';
    }
    return "";
  }

  // 2. Check timestamp delta
  ...
  if (tA != null && tB != null && !isNaN(tA) && !isNaN(tB)) {
    ...
    if (diffMs !== 0) {
      var sign = diffMs > 0 ? "+" : "";
      return '<span class="inline-flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-[var(--ink-noise-bg)] text-[var(--ink-noise)] border border-[var(--ink-noise)]" title="Timestamp shifted by ' + diffMs + 'ms">'
        + 'Δ ' + sign + diffMs.toLocaleString() + 'ms</span>';
    }
    return '<span class="text-[10px] font-mono text-[var(--text-muted)]">Δ 0ms</span>';
  }

  // 3. Check numeric delta
  if (typeof vA === "number" && typeof vB === "number") {
    ...
    if (Math.abs(d) > 0.0001) {
      ...
      var isSignificant = Math.abs(d / (vA || 1)) > 0.1;
      var colorClass = isSignificant ? "text-[var(--ink-semantic)] bg-[var(--ink-semantic-bg)] border-[var(--ink-semantic)]" : "text-[var(--ink-noise)] bg-[var(--ink-noise-bg)] border-[var(--ink-noise)]";
      return '<span class="inline-flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded border ' + colorClass + '">'
...
function renderDivergences(description) {
  ...
  if (semanticItems.length > 0) {
    html += '<div>'
      + '<div class="flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wider text-[var(--ink-semantic)] font-bold mb-1.5">'
    ...
    semanticItems.forEach(function (it) {
      var delta = formatDelta(it.rawA, it.rawB, it.field);
      ...
      html += '<div class="p-2.5 rounded-lg border border-[var(--ink-semantic)] bg-[var(--ink-semantic-bg)]">'
        + '<div class="flex items-center justify-between gap-2 mb-1.5 flex-wrap">'
        + '<span class="font-mono text-xs font-bold text-[var(--ink-semantic)]">' + esc(it.field) + '</span>'
        + '<div class="flex items-center gap-1.5">' + delta + ruleBadge + '</div>'
        + '</div>'
        + '<div class="grid grid-cols-2 gap-2 font-mono text-xs pt-1 border-t border-[var(--ink-semantic)]/20">'
  ...
  if (uncertainItems.length > 0) {
    html += '<div>'
      + '<div class="flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wider text-[var(--ink-uncertain)] font-bold mb-1.5">'
    ...
    uncertainItems.forEach(function (it) {
      var delta = formatDelta(it.rawA, it.rawB, it.field);
      html += '<div class="p-2.5 rounded-lg border border-dashed border-[var(--ink-uncertain)] bg-surface2">'
        + '<div class="flex items-center justify-between gap-2 mb-1.5 flex-wrap">'
        + '<span class="font-mono text-xs font-bold text-[var(--ink-uncertain)]">' + esc(it.field) + '</span>'
        + delta
        + '</div>'
  ...
  // 3. Noise items (judged safe)
  if (noiseItems.length > 0) {
    html += '<div>'
      + '<div class="flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wider text-[var(--ink-noise)] font-semibold mb-1.5">'
    ...
    noiseItems.forEach(function (it) {
      ...
      html += '<div class="p-2 rounded-lg border border-edge bg-surface2 hover:border-[var(--border-strong)] transition-colors">'
        ...
        + '<span class="font-mono text-xs text-[var(--ink-noise)]">' + esc(it.field) + '</span>'
        + '<div class="flex items-center gap-1.5">' + delta + ruleBadge + '</div>'
        + '</div>'
        + '<div class="grid grid-cols-2 gap-2 font-mono text-xs pt-1 border-t border-[var(--border)]/40">'
...
function attrTable(nodeA, nodeB, highlight) {
  ...
  // Both nodeA and nodeB exist: Unified 3-column diff table
  var changedCount = 0;
  var tableRows = allKeys.map(function (k) {
    ...
    var delta = isChanged ? formatDelta(vA, vB, k) : "";
    var rowClass = isChanged
      ? 'border-b border-edge bg-[var(--ink-semantic-bg)] hover:bg-[var(--surface-raised)] transition-colors'
    ...
    return '<tr class="' + rowClass + '">'
      ...
      + esc(k)
      + (isChanged ? '<span class="inline-block w-1.5 h-1.5 rounded-full bg-[var(--ink-semantic)] ml-1.5 align-middle"></span>' : '')
      + '</td>'
...
function renderDetail() {
  ...
  if (sig === "uncertain") {
    divergencesHtml += '<div class="mt-3 border border-dashed border-[var(--ink-uncertain)] rounded-lg p-3 font-mono text-[11px] text-[var(--ink-uncertain)] bg-[var(--surface-raised)]">'
  ...
  if (r.type === "added") {
    attrs = '<div class="mt-5 pt-3 border-t border-edge">'
      ...
      + '<span class="text-[var(--ink-added)] font-bold">+' + r.affectedSubtreeSize + ' descendant spans</span>'
      + '</div>'
    ...
    attrs = '<div class="mt-5 pt-3 border-t border-edge">'
      ...
      + '<span class="text-[var(--ink-removed)] font-bold">removed</span>'
      + '</div>'
    ...
    attrs = '<div class="mt-5 pt-3 border-t border-edge">'
      ...
      + attrTable(r.nodeA, r.nodeB, true)
      + '</div>';
  }
