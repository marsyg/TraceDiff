...
[← Open TraceDiff App](../) · [Home](index.html) · [Architecture](architecture.html) · **Developers** · [Hash experiment](hash-experiment.html) · [Engineering log](engineering-log.html)
...
## 7. Cloud Architecture (AWS Serverless Engine)

For the complete distributed cloud architecture, two-phase pipeline diagrams, direct S3 presigned upload flows, Step Functions Map-State concurrency, dual-table DynamoDB schemas, and the FinOps cost engine, see the dedicated [Visual Architecture Guide](architecture.html#12-distributed-cloud-architecture-aws-serverless-engine).
...
<script>
  (function() {
    ...
    // Re-render when details dropdown is toggled open
